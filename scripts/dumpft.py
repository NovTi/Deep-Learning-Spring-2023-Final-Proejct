...
dump = []
root = f'{sys.argv[1]}'
column_size = [12, 40, 10, 20, 15, 15]
column_name = ['slurm_id', 'exp_id', 'epoch', 'loss', 'train miou', 'best miou']
...
for i, d in enumerate(sorted(file_lst)):
    ...
    if len(train_lines) == 0 or exp_id is None or len(validate_lines) == 0:
        results = [slurm_id, 'None', 'None', 'None', 'None', 'None']
    else:
        train_info = train_lines[-1].split(' ')
        epoch = train_info[train_info.index('Epoch')+1]
        loss = '{} {}'.format(train_info[-2], train_info[-1])
        train_miou = train_info[16]
        
        best_miou = validate_lines[-1].split(' ')[-1]
        results = [slurm_id, exp_id, epoch, loss, train_miou, best_miou]
    # print results
